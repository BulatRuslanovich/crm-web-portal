...
import {
  ...
  Label, Input, Select, Textarea, ErrorBox,
...
export default function ActivEditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: allDrugs = [] } = useApi(() => drugsApi.getAll(1, 200).then(({ data }) => data.items), []);
    ...
    <div className="max-w-2xl mx-auto space-y-4">
      ...
      <form action={handleUpdate}>
        <Card>
          ...
          <CardFooter>
            <BtnSecondary type="button" onClick={() => router.push(`/activs/${id}`)}>Отмена</BtnSecondary>
