...
export default function ActivsPage() {
  ...
  return (
    <PageTransition className="max-w-4xl mx-auto space-y-4">
              ...
              <button
                ...
                className={`px-2.5 py-1 text-xs rounded-full border transition-all cursor-pointer ${
                  active
                    ...
                    : 'bg-(--surface) text-(--fg-muted) border-(--border) hover:border-(--primary-border) hover:text-(--fg)'
                }`}
              >
                {s.statusName}
              </button>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="flex items-center justify-between text-xs text-(--fg-muted) px-1">
            <span>{activs.length} визит(ов)</span>
          </div>
